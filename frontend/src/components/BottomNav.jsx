...
import { Bell, Clock, Home, UserRound, ScanLine } from 'lucide-react';

const tabs = [
  { to: '/', label: 'Home', icon: Home, hasNotification: true },
  { to: '/alerts', label: 'Alerts', icon: Bell, hasNotification: true },
  { to: '/history', label: 'History', icon: Clock, hasNotification: false },
  { to: '/profile', label: 'Profile', icon: UserRound, hasNotification: true },
];
    ...
    <nav className="sticky bottom-0 z-30 px-2 pb-3 pt-1">
      <div className="relative mx-auto flex max-w-[420px] items-center justify-between overflow-visible rounded-full border border-[#4c2c97]/15 bg-gradient-to-r from-white/95 via-[#f7f1ff]/95 to-white/95 px-2 py-2 shadow-[0_-8px_30px_-12px_rgba(76,44,151,0.45),0_18px_45px_-24px_rgba(76,44,151,0.65)] backdrop-blur-xl">
        {tabs.map((tab) => {
          const Icon = tab.icon;
          return (
            <div key={tab.to} className="flex-1">
              <NavLink
                to={tab.to}
                className={({ isActive }) =>
                  `flex flex-col items-center justify-center rounded-full px-2 py-2 text-[11px] font-semibold transition-all ${
              ...
              >
                {({ isActive }) => (
                  <div className="relative">
                    ...
                    {tab.hasNotification ? (
                      <span className="absolute right-0.5 -top-1 h-2.5 w-2.5 rounded-full bg-rose-500 ring-2 ring-white" />
                    ) : null}
                    <span className={`mt-1 ${isActive ? 'text-indigo-600' : 'text-gray-400'}`}>{tab.label}</span>
                  </div>
                )}
              </NavLink>
            </div>
        ...
        {!hideScanFab ? (
          <div className="absolute left-1/2 top-1/5 -translate-x-1/2 -translate-y-1/2">
            <motion.div
              initial={{ scale: 1 }}
